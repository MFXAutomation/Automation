...
import funClass.functions.Billing;
...
public class Billings extends BaseTest {
	...
	@Test(groups = { "Regression", "Billings", "AL" })
	public void billingProcess(@Optional("local") String executionLocation,
...
//		transType=testParams.sTestName;
		testParams.sCurrClass = "AL";
				...
				testParams.dataFilePath = System.getProperty("user.dir") + "\\src\\main\\resources\\Data\\TestDataBilling.xlsx";
		String className = "Billings";
...
//		iterationCount=String.valueOf(iterCount);
		...
		for (int gIter = 0; gIter < 1; gIter++) {
			...
			try {
				...
				iTestStatus += Billing.billProcess(testParams, "Billing");
				Reporting.PrintLn(testParams, "Ending - Billing.billProcess() - status = " + iTestStatus);
				
				iTestStatus += Billing.changePlan(testParams, "ChangePayPlan");
				Reporting.PrintLn(testParams, "Ending - Billing.changePlan() - status = " + iTestStatus);
				
				iTestStatus += Billing.creditAdjust(testParams, "CreditAdjust");
				Reporting.PrintLn(testParams, "Ending - Billing.creditAdjust() - status = " + iTestStatus);
				
				iTestStatus += Billing.adjustment(testParams, "Adjustment");
				Reporting.PrintLn(testParams, "Ending - Billing.adjustment() - status = " + iTestStatus);
